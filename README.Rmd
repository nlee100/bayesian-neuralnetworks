...
```{r}
#top regulated genes
upregulated<-tT[which(tT$logFC>0),][1:5,]
#upregulated  ##use this to get gene details e.g GO function
upregulated<-subset(upregulated, select=c("Gene.ID","", "logFC","AveExpr","t","P.Value","adj.P.Val","B"))
#View(upregulated)

#write.table(upregulated, file="Top 20 Upregulated Genes.txt",row.names=F, quote=FALSE, sep=",")

##getting top 10 downregulated genes
downreg<-tT[which(tT$logFC<0),][1:5,]
downreg<-subset(downreg, select=c("Gene.ID","logFC","AveExpr","t","P.Value","adj.P.Val","B"))
#View(downreg)
